...
rule Neo23x0_gen_Ping_Command_in_EXE {
   meta:
...
rule Neo23x0_gen_GoogleBot_UserAgent {
   meta:
...
rule Neo23x0_gen_Gen_Net_LocalGroup_Administrators_Add_Command {
   meta:
...
rule Neo23x0_gen_Suspicious_Script_Running_from_HTTP {
   meta:
...
rule Neo23x0_gen_ReconCommands_in_File : FILE {
   meta:
...
rule Neo23x0_gen_VBS_dropper_script_Dec17_1 {
   meta:
...
rule Neo23x0_gen_SUSP_PDB_Strings_Keylogger_Backdoor : HIGHVOL {
   meta:
...
rule Neo23x0_gen_SUSP_Microsoft_Copyright_String_Anomaly_2 {
   meta:
...
rule Neo23x0_gen_SUSP_LNK_File_AppData_Roaming {
   meta:
...
rule Neo23x0_gen_SUSP_LNK_File_PathTraversal {
   meta:
...
rule Neo23x0_gen_SUSP_Script_Obfuscation_Char_Concat {
   meta:
...
rule Neo23x0_gen_SUSP_PowerShell_IEX_Download_Combo {
   meta:
      description = "Detects strings found in sample from CN group repo leak in October 2018"
      author = "Florian Roth (Nextron Systems)"
      reference = "https://twitter.com/JaromirHorejsi/status/1047084277920411648"
...
rule Neo23x0_gen_SUSP_Win32dll_String {
   meta:
...
rule Neo23x0_gen_SUSP_Modified_SystemExeFileName_in_File {
   meta:
...
rule Neo23x0_gen_SUSP_JAVA_Class_with_VBS_Content {
   meta:
...
rule Neo23x0_gen_SUSP_RAR_with_PDF_Script_Obfuscation {
   meta:
...
rule Neo23x0_gen_SUSP_Netsh_PortProxy_Command {
   meta:
...
rule Neo23x0_gen_SUSP_DropperBackdoor_Keywords {
   meta:
...
rule Neo23x0_gen_SUSP_XMRIG_Reference {
   meta:
...
rule Neo23x0_gen_SUSP_Just_EICAR {
   meta:
...
rule Neo23x0_gen_SUSP_PDB_Path_Keywords {
   meta:
...
rule Neo23x0_gen_SUSP_Disable_ETW_Jun20_1 {
   meta:
...
rule Neo23x0_gen_SUSP_PE_Discord_Attachment_Oct21_1 {
   meta:
...
rule Neo23x0_gen_SUSP_Encoded_Discord_Attachment_Oct21_1 {
   meta:
