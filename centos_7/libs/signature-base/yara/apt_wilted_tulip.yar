...
rule Neo23x0_apt_WiltedTulip_Tools_back {
   meta:
...
rule Neo23x0_apt_WiltedTulip_Tools_clrlg {
   meta:
...
rule Neo23x0_apt_WiltedTulip_powershell {
   meta:
...
rule Neo23x0_apt_WiltedTulip_vminst {
   meta:
...
rule Neo23x0_apt_WiltedTulip_Windows_UM_Task {
   meta:
...
rule Neo23x0_apt_WiltedTulip_WindowsTask {
   meta:
...
rule Neo23x0_apt_WiltedTulip_tdtess {
   meta:
...
rule Neo23x0_apt_WiltedTulip_SilverlightMSI {
   meta:
...
rule Neo23x0_apt_WiltedTulip_matryoshka_Injector {
   meta:
...
rule Neo23x0_apt_WiltedTulip_Zpp {
   meta:
      description = "Detects hack tool used in Operation Wilted Tulip"
      license = "Detection Rule License 1.1 https://github.com/Neo23x0/signature-base/blob/master/LICENSE"
      author = "Florian Roth (Nextron Systems)"
...
rule Neo23x0_apt_WiltedTulip_Netsrv_netsrvs {
   meta:
...
rule Neo23x0_apt_WiltedTulip_ReflectiveLoader {
   meta:
...
rule Neo23x0_apt_WiltedTulip_Matryoshka_RAT {
   meta:
